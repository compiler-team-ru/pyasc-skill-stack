...
| `golden/kernels/gelu_f32.py` | `asc2.tanh + mul + add / float32` | `kernels/test_gelu.py` (2D row-tiled, f32, erf+tanh parametrized, `rtol=1e-3, atol=1e-5`) | *(n/a; covered in test_unary_ops for the individual ops)* | `target/test_gelu.py` (1D flatten, **tanh form**, f32 at `rtol=1e-3, atol=1e-3`) | Form (tanh/Padé) matches upstream `target/`. Tolerance `atol=rtol=1e-2` is **looser** than upstream `target/` (`1e-3`) — likely safe headroom but worth tightening in a future sprint after a stability sweep. numpy interface. |
