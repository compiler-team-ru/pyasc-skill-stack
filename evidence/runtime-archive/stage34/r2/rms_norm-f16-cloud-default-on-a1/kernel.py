...
"""
pyasc kernel: rms_norm_float16

Operation: RMSNorm - Root Mean Square Layer Normalization
y = x * gamma * rsqrt(mean(x², axis=-1, keepdims=True) + eps)
...
def _full_row_launch(x: torch.Tensor, gamma: torch.Tensor, eps: float, core_num: int) -> torch.Tensor:
...
def _split_d_launch(x: torch.Tensor, gamma: torch.Tensor, eps: float, core_num: int) -> torch.Tensor:
    num_rows, num_cols = x.shape
    tile_cols = 64
...
def rms_norm_launch(x: torch.Tensor, gamma: torch.Tensor, eps: float = 1e-5, core_num: int = CORE_NUM) -> torch.Tensor:
    num_rows, num_cols = x.shape
...
def torch_rms_norm(x: torch.Tensor, gamma: torch.Tensor, eps: float = 1e-5) -> torch.Tensor:
...
def run_kernel(backend: config.Backend, platform: config.Platform):
    ...
    test_cases = [
        (8, 256, "full_row", 2e-2, 2e-2),
        (8, 1055, "split_d", 5e-2, 5e-2),
    ]
    
    for num_rows, num_cols, kernel_type, atol, rtol in test_cases:
        x = torch.rand((num_rows, num_cols), dtype=torch.float16, generator=rng) * 2 - 1
        gamma = torch.rand((num_cols,), dtype=torch.float16, generator=rng) * 2 - 1
        eps = 1e-5
        
        out = rms_norm_launch(x, gamma, eps, core_num=CORE_NUM)
        ...
        logging.info(f"[PASS] Kernel output verified for shape ({num_rows}, {num_cols}) - {kernel_type} kernel")
