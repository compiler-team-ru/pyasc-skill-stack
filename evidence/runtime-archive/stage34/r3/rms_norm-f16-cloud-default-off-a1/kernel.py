...
"""RMSNorm float16 with host-side dispatcher (full_row/split_d).

Cell metadata:
  - shape_regime: dynamic
  - reduce_axis: -1
...
Two @asc2.jit kernels:
  - full_row: num_cols is ConstExpr[int], row fits in UB
  - split_d: num_rows/num_cols runtime int, streams along D in tile_cols=64

Host dispatcher picks full_row when num_cols*dtype_bytes <= 64KB AND
num_cols % 8 == 0, otherwise split_d.

Test shapes: (8, 256) for full_row, (8, 1055) for split_d.
"""
...
def rms_norm_full_row_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
    ...
    """Full-row variant for float16. Accumulator stays in float32."""
...
def rms_norm_split_d_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
                            ...
                            epsilon: asc.ConstExpr[float]):
    """Split-D variant for float16. Accumulator in float32; output cast back."""
...
def rms_norm_launch(x: torch.Tensor, gamma: torch.Tensor,
    ...
    """Host-side dispatcher between full_row and split_d variants."""
    num_rows, num_cols = x.shape
