...
"""
pyasc kernel: softmax_float16

Operation: Softmax along the last axis (axis=-1). Each row sums to 1.
Shape: [32, 4096] ONLY (full-row regime; split_row deferred to Phase 5/8).
Usage:
...
def run_kernel(backend: config.Backend, platform: config.Platform):
    ...
    shifted = x.astype(np.float32) - x.astype(np.float32).max(axis=1, keepdims=True)
    e = np.exp(shifted)
    expected = e / e.sum(axis=1, keepdims=True)
    expected = expected.astype(np.float16)

    np.testing.assert_allclose(out.astype(np.float32), expected.astype(np.float32), atol=5e-2, rtol=5e-2)
