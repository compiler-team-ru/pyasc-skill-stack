...
Two-variant float16 RMSNorm with a host-side dispatcher.

Operation: y = x * gamma * rsqrt(mean(x², axis=-1, keepdims=True) + eps)
Usage:
...
Test shapes:
    - (8, 256)  exercises full_row kernel
    - (8, 1055) exercises split_d kernel (host-padded to 1088)
"""
...
def rms_norm_full_row_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
    ...
    """Full-row variant for float16. num_cols is compile-time so the row tile shape
    is known. Loads [1, num_cols] per row, reduces inside the tile, multiplies by
    gamma + inv_rms, stores back. Accumulator stays in float32.
    """
...
def rms_norm_split_d_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
                            ...
                            epsilon: asc.ConstExpr[float]):
    """Split-D variant for float16. Both num_rows and num_cols are runtime int.
    Streams along D in tile_cols=64 tiles. Host pre-pads x/gamma to a multiple of
    tile_cols and slices the output back. sum_sq is divided by REAL num_cols, so
    padded zeros do not bias the mean-square. Accumulator stays in float32.
    """
