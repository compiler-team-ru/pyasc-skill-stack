...
"""RMSNorm kernel: float32 with host-side dispatcher.

Two-variant RMSNorm:
  - full_row: compile-time num_cols (asc.ConstExpr[int]), row fits in UB
  - split_d: runtime num_rows/num_cols, streams along D in tile_cols=64 tiles

Formula: y = x * gamma * rsqrt(mean(x², axis=-1, keepdims=True) + eps)

Test shapes:
  - (8, 256)  -> full_row (row fits in UB, 8-aligned)
  - (8, 1055) -> split_d (streams with host zero-padding to 1088)
...
def rms_norm_full_row_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
    ...
    """Full-row variant. num_cols is compile-time constant.
    
    Loads [1, num_cols] per row, computes RMSNorm, stores back.
    Uses block_idx to distribute rows across cores.
    """
...
def rms_norm_split_d_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
    ...
    Streams along D in tile_cols=64 tiles with host-side zero padding.
    Seeds sum_sq with PlainValue zero via full([1, tile_cols], 0.0).reduce_sum().
    Uses block_idx to distribute rows across cores.
    """
...
def rms_norm_launch(x: torch.Tensor, gamma: torch.Tensor,
    ...
    """Host-side dispatcher: picks full_row when row fits in UB and is 8-aligned."""
    num_rows, num_cols = x.shape
