...
"""
pyasc kernel: rms_norm (float32)

Operation: RMSNorm - Root Mean Square Layer Normalization
y = x * gamma * rsqrt(mean(x², axis=-1, keepdims=True) + eps)

This kernel implements two variants with a host-side dispatcher:
1. full_row: When row fits in UB and is 8-aligned, processes entire row in one tile
2. split_d: When row exceeds UB or is not aligned, streams along D in 64-element tiles
...
def rms_norm_full_row_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
    ...
    """Full-row variant. num_cols is compile-time so the row tile shape is known.
    
    Loads [1, num_cols] per row, reduces inside the tile,
    multiplies by gamma + inv_rms, stores back.
    Mirrors CANN's KernelRmsNormRegBase.
    """
...
def rms_norm_split_d_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
    ...
    Streams along D in tile_cols=64 (one Ascend SIMD lane) tiles;
    the host pre-pads x/gamma to a multiple of tile_cols and slices
    the output back. sum_sq is divided by REAL num_cols, so padded
    zeros do not bias the mean-square.
    Mirrors CANN's KernelRmsNormRegBaseSplitD.
    """
