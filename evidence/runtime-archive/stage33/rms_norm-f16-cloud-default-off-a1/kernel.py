#!/usr/bin/env python3
"""RMSNorm kernel for float16 with host-side dispatcher.

Two-variant float16 RMSNorm with a host-side dispatcher:
  - full_row: when num_cols fits in UB and is 8-aligned
  - split_d: streams along D in 64-element tiles with host-side zero padding

RMSNorm formula: y = x * gamma * rsqrt(mean(x², axis=-1, keepdims=True) + eps)
"""
...
def rms_norm_split_d_kernel(x_ptr: asc.GlobalAddress, gamma_ptr: asc.GlobalAddress,
                            ...
                            epsilon: asc.ConstExpr[float]):
    """Split-D variant for float16. Accumulator stays in float32."""
...
def rms_norm_launch(x: torch.Tensor, gamma: torch.Tensor,
    ...
    """Host-side dispatcher between the two variants for float16."""
    num_rows, num_cols = x.shape
